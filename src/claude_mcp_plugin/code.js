...
const state = {
  ...
  changeNotificationsEnabled: true, // Enable change notifications by default
  lastChangeNotification: 0, // Throttle notifications
  changeNotificationThrottleMs: 1000, // Minimum 1 second between notifications
};
...
function sendChangeNotification(changeType, details) {
  ...
  state.lastChangeNotification = now;
...
figma.on('selectionchange', () => {
  ...
  sendChangeNotification('selection_change', {
    selectionCount: selection.length,
    selectedIds: selection.slice(0, 10).map(n => n.id), // Limit to first 10
    selectedNames: selection.slice(0, 10).map(n => n.name)
  });
});

// Listen for page changes
figma.on('currentpagechange', () => {
  sendChangeNotification('page_change', {
    newPageId: figma.currentPage.id,
...
async function handleCommand(command, params) {
  switch (command) {
    ...
    // DaisyUI Edition - New commands
    case "extract_node_styles":
      return await extractNodeStyles(params);
    case "get_page_structure":
      return await getPageStructure(params);
    case "batch_get_nodes":
      return await batchGetNodes(params);
    // Prototype/Interaction commands
    case "set_prototype_link":
      return await setPrototypeLink(params);
    case "get_prototype_links":
      return await getPrototypeLinks(params);
...
// ============================================================
// DaisyUI Edition - New Functions
// ============================================================

/**
 * Extract styles from a node in a format optimized for Tailwind/DaisyUI mapping
 * Returns compact style data ready for server-side conversion
 */
async function extractNodeStyles(params) {
  const { nodeId, includeChildren = false, maxDepth = 3 } = params || {};
  ...
  const extractStyles = async (n, depth = 0) => {
    const styles = {
      id: n.id,
      name: n.name,
      type: n.type,
    ...
    // Position and size
    if ('x' in n) styles.x = Math.round(n.x);
    if ('y' in n) styles.y = Math.round(n.y);
    if ('width' in n) styles.width = Math.round(n.width);
    if ('height' in n) styles.height = Math.round(n.height);
    ...
    if ('fills' in n && Array.isArray(n.fills) && n.fills.length > 0) {
      ...
      if (fill && fill.type === 'SOLID' && fill.visible !== false) {
        styles.fill = {
          ...
          a: fill.opacity !== undefined ? Math.round(fill.opacity * 100) / 100 : 1,
        };
      }
    }
    ...
    if ('strokes' in n && Array.isArray(n.strokes) && n.strokes.length > 0) {
      ...
      if (stroke && stroke.type === 'SOLID' && stroke.visible !== false) {
        styles.stroke = {
          ...
          a: stroke.opacity !== undefined ? Math.round(stroke.opacity * 100) / 100 : 1,
    ...
    if ('cornerRadius' in n && n.cornerRadius !== figma.mixed && n.cornerRadius > 0) {
      styles.radius = n.cornerRadius;
    } else if ('topLeftRadius' in n) {
      const radii = [n.topLeftRadius, n.topRightRadius, n.bottomRightRadius, n.bottomLeftRadius];
      if (radii.some(r => r > 0)) {
        styles.radius = radii;
      }
    }
    ...
    if (n.type === 'TEXT') {
      try {
        styles.text = {
          ...
          fontFamily: n.fontName !== figma.mixed ? n.fontName.family : 'mixed',
          fontWeight: n.fontName !== figma.mixed ? n.fontName.style : 'mixed',
          textAlign: n.textAlignHorizontal,
          verticalAlign: n.textAlignVertical,
          lineHeight: n.lineHeight !== figma.mixed ? n.lineHeight : 'mixed',
          letterSpacing: n.letterSpacing !== figma.mixed ? n.letterSpacing : 'mixed',
        };
      } catch (e) {
        styles.text = { error: 'Could not extract text properties' };
      }
    }

    // Recursively process children if requested
    if (includeChildren && 'children' in n && depth < maxDepth) {
      styles.children = await Promise.all(
        n.children.map(child => extractStyles(child, depth + 1))
      );
    }
  ...
  const result = await extractStyles(node);
  return result;
}

/**
 * Get page structure in a compact format for indexing
 * Returns minimal data optimized for building search indices
 */
async function getPageStructure(params) {
  const { pageId, maxDepth = 5, includeHidden = false } = params || {};

  // Get the page (current page if not specified)
  const page = pageId
    ? await figma.getNodeByIdAsync(pageId)
    : figma.currentPage;

  if (!page) {
    throw new Error(`Page not found: ${pageId || 'current'}`);
  }

  const collectStructure = (node, path = '', depth = 0) => {
    ...
    const item = {
      id: node.id,
      n: node.name, // short key for compactness
      t: node.type, // short key for compactness
      p: nodePath,
      d: depth,
    };

    // Add position/size for frames and components
    if (['FRAME', 'COMPONENT', 'INSTANCE', 'GROUP'].includes(node.type)) {
      if ('width' in node && 'height' in node) {
        item.w = Math.round(node.width);
        item.h = Math.round(node.height);
      }
    }
    ...
    if (node.type === 'INSTANCE') {
      ...
      if ('mainComponent' in node && node.mainComponent) {
        item.mainComponentId = node.mainComponent.id;
      }
    }
    ...
    if ('children' in node && node.children.length > 0) {
      const children = node.children
        .map(child => collectStructure(child, nodePath, depth + 1))
        .filter(Boolean);
  ...
  if ('children' in page) {
    structure.nodes = page.children
      .map(child => collectStructure(child, page.name, 1))
      .filter(Boolean);
  }
  ...
  structure.totalNodes = countNodes(structure.nodes);
...
/**
 * Batch get multiple nodes with minimal data
 * Optimized for building local indices
 */
async function batchGetNodes(params) {
  const { nodeIds, fields = ['id', 'name', 'type'] } = params || {};
  ...
  const idsToFetch = nodeIds.slice(0, BATCH_LIMIT);
  ...
  const results = await Promise.all(
    idsToFetch.map(async (id) => {
      try {
        ...
        if (fields.includes('name')) data.name = node.name;
        if (fields.includes('type')) data.type = node.type;
        if (fields.includes('visible') && 'visible' in node) data.visible = node.visible;
        if (fields.includes('position') && 'x' in node) {
          data.x = Math.round(node.x);
          data.y = Math.round(node.y);
        }
        if (fields.includes('size') && 'width' in node) {
          data.width = Math.round(node.width);
          data.height = Math.round(node.height);
        }
        if (fields.includes('parent') && node.parent) {
  ...
  return {
    ...
    truncated: nodeIds.length > BATCH_LIMIT,
    originalCount: nodeIds.length,
  };
}

// ============================================================
// Prototype / Interaction Link Functions
// ============================================================

/**
 * Create a prototype link from a node to a destination frame
 * Enables interactive prototypes where clicking links to another page/frame
 */
async function setPrototypeLink(params) {
  const {
    nodeId,
    destinationId,
    trigger = 'ON_CLICK',
  ...
  if (!destinationId) {
    throw new Error("Missing destinationId parameter (target frame to navigate to)");
  }
  ...
  // Check if node supports reactions (interactions)
  if (!('reactions' in node)) {
    throw new Error(`Node does not support prototype interactions: ${nodeId}`);
  }

  // Valid triggers: ON_CLICK, ON_HOVER, ON_PRESS, ON_DRAG, AFTER_TIMEOUT, MOUSE_ENTER, MOUSE_LEAVE, MOUSE_UP, MOUSE_DOWN
  const validTriggers = ['ON_CLICK', 'ON_HOVER', 'ON_PRESS', 'ON_DRAG', 'AFTER_TIMEOUT', 'MOUSE_ENTER', 'MOUSE_LEAVE', 'MOUSE_UP', 'MOUSE_DOWN'];
  if (!validTriggers.includes(trigger)) {
    throw new Error(`Invalid trigger: ${trigger}. Must be one of: ${validTriggers.join(', ')}`);
  }

  // Valid transitions: INSTANT, DISSOLVE, SMART_ANIMATE, MOVE_IN, MOVE_OUT, PUSH, SLIDE_IN, SLIDE_OUT
  const validTransitions = ['INSTANT', 'DISSOLVE', 'SMART_ANIMATE', 'MOVE_IN', 'MOVE_OUT', 'PUSH', 'SLIDE_IN', 'SLIDE_OUT'];
  ...
  // Valid easings: LINEAR, EASE_IN, EASE_OUT, EASE_IN_AND_OUT, EASE_IN_BACK, EASE_OUT_BACK, EASE_IN_AND_OUT_BACK
  const validEasings = ['LINEAR', 'EASE_IN', 'EASE_OUT', 'EASE_IN_AND_OUT', 'EASE_IN_BACK', 'EASE_OUT_BACK', 'EASE_IN_AND_OUT_BACK'];
  if (!validEasings.includes(easing)) {
    throw new Error(`Invalid easing: ${easing}. Must be one of: ${validEasings.join(', ')}`);
  }

  // Create the reaction (prototype interaction)
  const reaction = {
    ...
    action: {
      ...
      navigation: 'NAVIGATE',
      transition: transition === 'INSTANT' ? null : {
        type: transition,
        duration: duration,
        easing: { type: easing },
      },
  ...
  node.reactions = [...(node.reactions || []), reaction];
  ...
  return {
    ...
    transition,
    duration,
    totalReactions: node.reactions.length,
  };
}

/**
 * Get all prototype links from a node
 */
async function getPrototypeLinks(params) {
  const { nodeId, recursive = false, maxDepth = 3 } = params || {};
  ...
  const collectLinks = async (n, depth = 0) => {
    const links = [];

    // Get reactions from this node
    if ('reactions' in n && n.reactions && n.reactions.length > 0) {
      for (const reaction of n.reactions) {
        if (reaction.action && reaction.action.type === 'NODE' && reaction.action.destinationId) {
          const dest = await figma.getNodeByIdAsync(reaction.action.destinationId);
          links.push({
            sourceId: n.id,
            sourceName: n.name,
    ...
    if (recursive && 'children' in n && depth < maxDepth) {
      for (const child of n.children) {
        const childLinks = await collectLinks(child, depth + 1);
  ...
  const links = await collectLinks(node);
...
/**
 * Remove prototype links from a node
 */
async function removePrototypeLink(params) {
  const { nodeId, destinationId, removeAll = false } = params || {};
...
async function setScrollBehavior(params) {
  const {
    nodeId,
    scrollDirection = 'NONE', // NONE, HORIZONTAL, VERTICAL, BOTH
    scrollingFrames = 'SCROLLS', // SCROLLS, FIXED
  } = params || {};
  ...
  if (node.type !== 'FRAME') {
    throw new Error(`Node must be a FRAME to set scroll behavior: ${nodeId} is ${node.type}`);
  }

  // Set overflow behavior based on scroll direction
  if (scrollDirection === 'NONE') {
    node.overflowDirection = 'NONE';
  } else if (scrollDirection === 'HORIZONTAL') {
    node.overflowDirection = 'HORIZONTAL_SCROLLING';
  } else if (scrollDirection === 'VERTICAL') {
    node.overflowDirection = 'VERTICAL_SCROLLING';
  } else if (scrollDirection === 'BOTH') {
    node.overflowDirection = 'HORIZONTAL_AND_VERTICAL_SCROLLING';
  }
 ...
 */
async function lockNode(params) {
