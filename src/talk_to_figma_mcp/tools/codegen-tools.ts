...
`;
  ...
  /**
   * Get index status for sync monitoring
   */
  server.tool(
    "index_status",
    "Get the current status of the Figma index, including last sync time, node counts, and sync health.",
    {},
    async () => {
      try {
        cache.initialize();
        ...
        const documentInfo = cache.getMeta('document_name');

        const lastSyncTime = lastSync ? new Date(lastSync) : null;
        const syncAge = lastSyncTime ? (Date.now() - lastSyncTime.getTime()) / 1000 / 60 : null; // minutes

        return {
          content: [{
            type: "text",
            text: JSON.stringify({
              status: stats.node_count > 0 ? 'indexed' : 'empty',
              document: documentInfo || 'unknown',
              stats: {
              ...
              sync: {
                ...
                sync_age_minutes: syncAge ? Math.round(syncAge) : null,
                is_stale: syncAge ? syncAge > 5 : true,
                auto_sync_enabled: true
              },
              health: syncAge && syncAge < 60 ? 'healthy' : syncAge && syncAge < 1440 ? 'stale' : 'needs_sync'
            }, null, 2)
          }]
        };
      } catch (error) {
