...
type ChangeNotificationCallback = (notification: ChangeNotification) => void;
const changeNotificationCallbacks: ChangeNotificationCallback[] = [];

// Change notification type
export interface ChangeNotification {
  type: 'change_notification';
  changeType: 'document_change' | 'selection_change' | 'page_change';
  timestamp: number;
  documentName: string;
  currentPage: string;
 ...
 */
export function onChangeNotification(callback: ChangeNotificationCallback): () => void {
  changeNotificationCallbacks.push(callback);
  // Return unsubscribe function
  return () => {
    const index = changeNotificationCallbacks.indexOf(callback);
    if (index > -1) {
      changeNotificationCallbacks.splice(index, 1);
    }
  };
}
 ...
 */
function notifyChangeCallbacks(notification: ChangeNotification): void {
  for (const callback of changeNotificationCallbacks) {
    try {
      callback(notification);
    } catch (error) {
      logger.error(`Error in change notification callback: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
}
  ...
  try {
    ...
    ws.on("message", (data: any) => {
      try {
        ...
        } else if (myResponse.type === 'change_notification') {
          // Handle Figma change notifications
          logger.info(`Received change notification: ${myResponse.changeType}`);
          notifyChangeCallbacks(myResponse as ChangeNotification);
        } else {
          // Handle other broadcast messages or events
...
/**
 * Check if connected to Figma and joined a channel.
 * @returns True if connected to WebSocket and joined a channel
 */
export function isConnected(): boolean {
  return ws !== null && ws.readyState === WebSocket.OPEN && currentChannel !== null;
}

/**
 * Check WebSocket connection status only (without channel requirement).
